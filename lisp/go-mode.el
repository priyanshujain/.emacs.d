...
âgo-dangling-operators-regexp.â, and must be updated whenever
...
Consider using âgodoc-gogetdocâ instead for more accurate results."
    (condition-case nil
...
The tool used can be set via âgofmt-command` (default: gofmt) and additional
arguments can be set as a list via âgofmt-args`."
    (interactive)
...
Note that this will cause âgo-modeâ to get loaded the first time
